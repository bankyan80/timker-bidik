...
function calculateBUP(tanggalLahir) {
...
async function main() {
  ...
  const employees = await DB.execute("SELECT id, nama, tanggal_lahir, tanggal_bup, status_pegawai FROM employees WHERE tanggal_lahir IS NOT NULL AND tanggal_lahir != ''");
  ...
  for (const emp of employees.rows) {
    const bup = calculateBUP(emp.tanggal_lahir);
    ...
    if (emp.tanggal_bup !== bup) {
      ...
      console.log('  ' + emp.nama + ' (' + emp.tanggal_lahir + ') -> BUP: ' + bup + (emp.tanggal_bup ? ' (was: ' + emp.tanggal_bup + ')' : ''));
      updated++;
    } else {
      skipped++;
    }
