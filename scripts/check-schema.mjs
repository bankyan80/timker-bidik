...
for (const t of ['schools', 'employees', 'employee_documents']) {
  const r = await db.execute(`SELECT sql FROM sqlite_master WHERE type='table' AND name='${t}'`);
  if (r.rows[0]) console.log(t + ':\n' + r.rows[0].sql + '\n');
}
process.exit(0);
